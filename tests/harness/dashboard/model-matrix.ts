 ...
 * agent runner (claude / codex); `model` is passed through to the CLI
 * with `--model` so the same suite can be graded
 * across model tiers.
 */
 ...
 * naming the `codex` CLI accepts. Fable 5 is intentionally banned from this benchmark because live
