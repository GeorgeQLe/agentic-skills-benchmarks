...
function qualityRoutes(definition: BaseWorkflowDefinition): string | string[] | undefined {
  ...
  if (routes.length > 0) {
    // De-dupe so Claude/Grok shared slash routes don't inflate the variant list.
...
const baseWorkflowDefinitions: BaseWorkflowDefinition[] = [
  ...
  },
  {
    ...
    recommendedRoutes: recommendedRoutesFor(
  ...
  },
  {
    ...
    recommendedRoutes: recommendedRoutesFor("idea-scope-brief poketo-teams"),
    requireFinalRecommendedRoute: true,
    requireExactFinalRecommendedRoute: true,
  },
  {
  ...
  },
  {
    ...
    recommendedRoutes: recommendedRoutesFor("bootstrap-repo --reset-existing"),
  ...
  },
  {
    ...
    prompt: "You have the icon-handler skill installed. Audit the Next App Router fixture using local file inspection tools and write icon-audit.md with framework, source asset, missing/stale icon surfaces, proposed fix, approval requirement, verification commands, and Next command. Write icon-audit.md before the final response. Build commands belong under verification commands only after an approved fix; the final recommended next command must be the icon-handler fix approval route for this runner (/icon-handler fix calc-mascot-icon.svg for Claude or Grok, $icon-handler fix calc-mascot-icon.svg for Codex). Do not modify files. Do not call external image generation or image-analysis services.",
    fixtureFiles: {
    ...
    recommendedRoutes: recommendedRoutesFor("icon-handler"),
    requireFinalRecommendedRoute: true,
    perRunBudgetUsd: BENCH_BUDGETS_USD.standard,
  },
  {
